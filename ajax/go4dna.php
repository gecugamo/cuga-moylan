<!-- go4dna modal content -->
<div id="modal__go4dna" class="modal__content">
	<div class="modal">
		<header class="modal__header">
				<h4 class="modal__heading">go4dna.com</h4>
				<a class="modal__close" href="#0"></a>
		</header>
		<div class="modal__body">
			<div class="modal__info">
				<img class="modal__img" src="img/go4dna_2.jpg" alt="Screenshot from go4dna.com" />
				<p class="modal__summary">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Obcaecati, laborum aspernatur? Nihil dicta labore, voluptate cumque.</p>
				<a class="btn" href="http://go4dna.com" target="_blank" rel="noopener noreferrer">Visit Site</a>
			</div>
		</div>
	</div>
</div>
<!-- go4dna modal content -->
